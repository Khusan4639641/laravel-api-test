    ...
    <div className={cn("group relative overflow-hidden rounded-3xl border border-safi-border/80 bg-white p-6 shadow-[0_18px_48px_rgba(11,23,18,0.06)] transition-transform duration-200 hover:-translate-y-0.5", className)}>
      <div className="relative z-10 flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="safi-numeric font-serif text-2xl font-semibold leading-tight text-safi-green">{value}</div>
          {trend && <div className="safi-numeric mt-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-green-600">{trend}</div>}
        </div>
...
export function AdminTable({ headers, children }: { headers: string[], children: React.ReactNode }) {
  return (
    <div className="safi-table-shell flex flex-col overflow-hidden rounded-3xl border border-safi-border/80 bg-white shadow-[0_18px_48px_rgba(11,23,18,0.06)]">
      <div className="safi-table-scroll">
        <table className="safi-numeric w-full text-left border-collapse">
